...
class VersionInfo:
    ...
    '''
    def __init__(self):
        self.name = "mozillapackager_univ"
        self.version = "2.2"
...
Version: ''' + self.releaseVersion + '''-0ubuntu''' + self.options.debversion +'''-''' +self.options.localization+'''
...
Categories=''' + self.Categories)
    ...
    def cleanup(self):
        ...
        if self.ans == 'n':
            ...
            self.util.execSystemCommand(executionstring="sudo rm -rf /tmp/"+self.options.package+'debbuild')
            #os.remove(os.path.join('/tmp/'+self.options.package+'debbuild'))
...
class FirefoxInstaller(MozillaInstaller):
    ...
    def downloadPackage(self): # done, self.packageFilename
        ...
        self.util.robustDownload(argsdict={'executionstring':"wget -c --tries=5 --read-timeout=20 --waitretry=10 ftp://" + "%mirror%" + self.options.package + "/releases/" + self.releaseVersion + "/linux-i686/"+self.options.localization+"/" + self.packageFilename, 'includewithtest':True})
...
class SeamonkeyInstaller(MozillaInstaller):
    ...
    def getLatestVersion(self):
        MozillaInstaller.getLatestVersion(self)
        self.releaseVersion = self.util.getSystemOutput(executionstring="wget -c --tries=20 --read-timeout=60 --waitretry=10 -q -nv -O - http://www.seamonkey-project.org/ |grep 'product=seamonkey' -m 1", numlines=1, errormessage="Failed to retrieve the latest version of "+ self.options.package.capitalize())
