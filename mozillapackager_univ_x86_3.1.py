...
##            self.cleanup()
    ...
    def getMD5Sum(self): # done, self.sigFilename
        ...
        self.util.robustDownload(argsdict={'executionstring':"wget -c --tries=5 --read-timeout=20 --waitretry=10 -q -nv -O - http://" + "%mirror%" + self.options.package + "/releases/" + self.releaseVersion + "/MD5SUMS | grep -F 'linux-i686/"+self.options.localization+"/" + self.packageFilename + "' > " + self.sigFilename, 'includewithtest':True}, errormsg="Failed to retrieve md5 sum. This may be due to transient network problems, so try again later. Exiting.")

        # example: 91360c07aea125dbc3e03e33de4db01a  ./linux-i686/"+self.options.localization+"/seamonkey-2.0.tar.bz2
        # sed to:  91360c07aea125dbc3e03e33de4db01a  ./seamonkey-2.0.tar.bz2
        print("demunging: sed -i 's#linux-i686/"+self.options.localization+"/##' " + self.sigFilename + "...\n")
        self.util.execSystemCommand("sed -i 's#linux-i686/"+self.options.localization+"/##' " + self.sigFilename, includewithtest=True)
...
class FirefoxInstaller(MozillaInstaller):
    ...
    def downloadPackage(self): # done, self.packageFilename
        ...
        self.util.robustDownload(argsdict={'executionstring':"wget -c --tries=5 --read-timeout=20 --waitretry=10 http://" + "%mirror%" + self.options.package + "/releases/" + self.releaseVersion + "/linux-i686/"+self.options.localization+"/" + self.packageFilename, 'includewithtest':True})
...
class ThunderbirdInstaller(MozillaInstaller):
    ...
    def getLatestVersion(self):
        MozillaInstaller.getLatestVersion(self)
        self.releaseVersion = self.util.getSystemOutput(executionstring="wget -c --tries=20 --read-timeout=60 --waitretry=10 -q -nv -O - http://www.mozilla.org/it/thunderbird/ |grep 'product=thunderbird' | grep linux -m 1", numlines=1, errormessage="Failed to retrieve the latest version of "+ self.options.package.capitalize())
