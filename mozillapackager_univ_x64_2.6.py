...
class BaseStarter:
    ...
    def ParseOptions(self):
        ...
        parser.set_defaults(debug=False, 
                ...
                skipgpg=True,
                unattended=False,
                debversion=subprocess.Popen("lsb_release -cs",shell=True,stdout=subprocess.PIPE).stdout.readline().replace('\n',''),
                localization="it",
                #skipbackup=False,
                debdir=os.getcwd(),
                targetdir="/opt",
...
##            self.cleanup()
    ...
    def downloadPackage(self): 
        ...
        for mirror in self.options.mirrors:
            try:
                self.packageFilename = self.util.getSystemOutput(executionstring="w3m -dump ftp://" + mirror + self.options.package + "/releases/" + self.releaseVersion + "/linux-x86_64/"+self.options.localization+"/ | grep '" + self.options.package + "' | grep -v '\.asc' |grep -v 'ftp://' | awk '{ print substr($0,index($0, \"" + self.options.package + "\"))}' | awk '{print $1}' | sed -e 's/\.*$//'", numlines=1)
...
class FirefoxInstaller(MozillaInstaller):
    ...
    def getLatestVersion(self):
        ...
        self.releaseVersion = self.util.getSystemOutput(executionstring="wget -c --tries=20 --read-timeout=60 --waitretry=10 -q -nv -O - http://www.mozilla.org/it/ |grep 'product=firefox' -m 1", numlines=1, errormessage="Failed to retrieve the latest version of "+ self.options.package.capitalize())
    ...
    def downloadPackage(self): # done, self.packageFilename
        ...
        self.util.robustDownload(argsdict={'executionstring':"wget -c --tries=5 --read-timeout=20 --waitretry=10 ftp://" + "%mirror%" + self.options.package + "/releases/" + self.releaseVersion + "/linux-x86_64/"+self.options.localization+"/" + self.packageFilename, 'includewithtest':True})
...
class ThunderbirdInstaller(MozillaInstaller):
    ...
    def getLatestVersion(self):
        MozillaInstaller.getLatestVersion(self)
        self.releaseVersion = self.util.getSystemOutput(executionstring="wget -c --tries=20 --read-timeout=60 --waitretry=10 -q -nv -O - http://www.mozilla.org/it/thunderbird/ |grep 'product=thunderbird' -m 1", numlines=1, errormessage="Failed to retrieve the latest version of "+ self.options.package.capitalize())
