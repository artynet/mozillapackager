...
class UtilityFunctions:
    ...
    def getSystemOutput(self, executionstring, numlines=1, errormessage="Previous command has failed to complete successfully. Exiting."):
        ...
        p = subprocess.Popen(executionstring, shell=True, stdin=subprocess.PIPE, universal_newlines=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, close_fds=True)
        returncode = p.wait()
...
class BaseStarter:
    ...
    def ParseOptions(self):
        ...
        parser.set_defaults(debug=False,
                ...
                skipgpg=True,
                unattended=False,
				debversion=subprocess.Popen(['lsb_release', '-cs'],shell=False,stdout=subprocess.PIPE, universal_newlines=True).stdout.readline().replace('\n',''),
                localization="it",
                #skipbackup=False,
                debdir=os.getcwd(),
                targetdir="/opt",
                mirrors=['ftp.mozilla.org/pub/',
...
##            self.cleanup()
    ...
    def downloadGPGSignature(self): # done, self.sigFilename
        ...
        self.util.robustDownload(argsdict={'executionstring':"wget -c --tries=5 --read-timeout=20 --waitretry=10 http://" + "%mirror%" + self.options.package + "/releases/" + self.releaseVersion + "/linux-x86_64/"+self.options.localization+"/" + self.sigFilename, 'includewithtest':True}, errormsg="Failed to retrieve GPG key. This may be due to transient network problems, so try again later. Exiting.")
    ...
    def getMD5Sum(self): # done, self.sigFilename
        ...
        self.util.robustDownload(argsdict={'executionstring':"wget -c --tries=5 --read-timeout=20 --waitretry=10 -q -nv -O - http://" + "%mirror%" + self.options.package + "/releases/" + self.releaseVersion + "/MD5SUMS | grep -F 'linux-x86_64/"+self.options.localization+"/" + self.packageFilename + "' > " + self.sigFilename, 'includewithtest':True}, errormsg="Failed to retrieve md5 sum. This may be due to transient network problems, so try again later. Exiting.")

        # example: 91360c07aea125dbc3e03e33de4db01a  ./linux-x86_64/"+self.options.localization+"/seamonkey-2.0.tar.bz2
        # sed to:  91360c07aea125dbc3e03e33de4db01a  ./seamonkey-2.0.tar.bz2
        print("demunging: sed -i 's#linux-x86_64/"+self.options.localization+"/##' " + self.sigFilename + "...\n")
        self.util.execSystemCommand("sed -i 's#linux-x86_64/"+self.options.localization+"/##' " + self.sigFilename, includewithtest=True)
...
Version: ''' + self.releaseVersion +'''-''' + self.options.debversion +'''-''' +self.options.localization+'''
...
Categories=''' + self.Categories)
    ...
    def createRepository(self):
        ...
        if self.ans == 'y':
            ...
            self.util.execSystemCommand('reprepro -S web -P extra -A amd64 -Vb ../mozilla-apt-repository includedeb all ./'+self.packagename+'_' + self.releaseVersion + '-0ubuntu' + self.options.debversion + '_amd64.deb')
        else:
    ...
    def cleanup(self):
        ...
        if self.ans == 'n':
            ...
            self.util.execSystemCommand(executionstring="rm -rf /tmp/"+self.options.package+'debbuild')
...
class FirefoxInstaller(MozillaInstaller):
    ...
    def getLatestVersion(self):
        ...
        self.releaseVersion = self.util.getSystemOutput(executionstring="wget -c --tries=20 --read-timeout=60 --waitretry=10 -q -nv -O - https://www.mozilla.org/it/ | grep 'latest' -m 1", numlines=1, errormessage="Failed to retrieve the latest version of "+ self.options.package.capitalize())
        self.releaseVersion = re.search(r'(([0-9]+\.)+[0-9]+)',self.releaseVersion).group(1)
    ...
    def downloadPackage(self): # done, self.packageFilename
        ...
        self.util.robustDownload(argsdict={'executionstring':"wget -c --tries=5 --read-timeout=20 --waitretry=10 http://" + "%mirror%" + self.options.package + "/releases/" + self.releaseVersion + "/linux-x86_64/"+self.options.localization+"/" + self.packageFilename, 'includewithtest':True})
...
class ThunderbirdInstaller(MozillaInstaller):
    ...
    def getLatestVersion(self):
        MozillaInstaller.getLatestVersion(self)
        self.releaseVersion = self.util.getSystemOutput(executionstring="wget -c --tries=20 --read-timeout=60 --waitretry=10 -q -nv -O - http://www.mozilla.org/it/thunderbird/ |grep 'product=thunderbird' | grep linux64 -m 1", numlines=1, errormessage="Failed to retrieve the latest version of "+ self.options.package.capitalize())
